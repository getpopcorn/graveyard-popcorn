...
public struct Movie: Media, Equatable {
    ...
    private init(_ map: Map) throws {
        if map.context is TraktContext {
            ...
            self.summary = ((try? map.value("overview")) ?? "No summary available.").removingHtmlEncoding
            ...
            self.summary = ((try? map.value("synopsis")) ?? "No summary available.").removingHtmlEncoding
        ...
        torrents.sort(by: { $0.quality < $1.quality })
    }
    
    public init(title: String = "Unknown", id: String = "tt0000000", tmdbId: Int? = nil, slug: String = "unknown", summary: String = "No summary available.", torrents: [Torrent] = [], subtitles: Dictionary<String, [Subtitle]> = [:], largeBackgroundImage: String? = nil, largeCoverImage: String? = nil) {
        self.title = title
        self.id = id
        self.tmdbId = tmdbId
        self.slug = slug
