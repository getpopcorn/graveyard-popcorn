...
public final class UpdateManager: NSObject {
    ...
    private func checkForUpdates(_ completion: ((_ success: Bool) -> Void)? = nil) {
        ...
        Alamofire.request("https://api.github.com/repos/getpopcorn/getpopcorn.github.io/releases").validate().responseJSON { (response) in
            ...
            if let latestRelease = sortedReleases.first,
                ...
                print(currentRelease, latestRelease)

                let alert = UIAlertController(title: "Update Available", message: String(format: "%@ version %@ of Popcorn is now available.", latestRelease.releaseType.rawValue, latestRelease.buildNumber), preferredStyle: .alert)
                ...
                alert.addAction(UIAlertAction(title: "Update", style: .default) { _ in
                    if isCydiaInstalled {
                        ...
                        let instructionsAlert = UIAlertController(title: "Sideloading Instructions", message: "Unfortunately, in-app updates are not available for un-jailbroken devices. Please use AltStore.", preferredStyle: .alert)
                        instructionsAlert.addAction(UIAlertAction(title: "OK", style: .default, handler: nil))
                        UIApplication.shared.windows[0].rootViewController!.present(instructionsAlert, animated: true, completion: nil)
                    }
                })
                completion?(true)
