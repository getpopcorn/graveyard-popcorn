...
extension TraktManager {
    ...
    public func loginViewController() -> UIViewController {
            ...
            let vc = SFSafariViewController(url: URL(string: Trakt.base + Trakt.auth + "/authorize?client_id=" + Trakt.apiKey + "&redirect_uri=Popcorn%3A%2F%2Ftrakt&response_type=code&state=\(state!)")!)
            vc.modalPresentationStyle = .fullScreen
            
            return vc
        #else
            return TraktAuthenticationViewController()
        #endif
    }
