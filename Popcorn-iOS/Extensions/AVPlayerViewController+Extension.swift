...
//  PopcornTime
//
//  Created by Jarrod Norwell on 27/5/20.
...
public extension AVPlayerViewController {
    ...
    fileprivate func addSubtitleLabel() {
        ...
        guard let _ = subtitleLabel else {
            ...
            let size = CGFloat(UserDefaults.standard.float(forKey: "defaultFontSize"))
            let name = UserDefaults.standard.string(forKey: "defaultFontFamily") ?? UIFont.systemFont(ofSize: size).familyName
            
            subtitleLabel?.font = UIFont(name: name, size: size)?.bold
            subtitleLabel?.textColor = UIColor.white
            subtitleLabel?.numberOfLines = 0
            subtitleLabel?.layer.shadowColor = UIColor.black.cgColor
            subtitleLabel?.layer.shadowOffset = CGSize(width: 1.0, height: 1.0)
            subtitleLabel?.layer.shadowOpacity = 0.16
            ...
            subtitleLabel?.leadingAnchor.constraint(equalTo: contentOverlayView!.leadingAnchor, constant: 20).isActive = true
            subtitleLabel?.bottomAnchor.constraint(equalTo: contentOverlayView!.bottomAnchor, constant: -32).isActive = true
            subtitleLabel?.trailingAnchor.constraint(equalTo: contentOverlayView!.trailingAnchor, constant: -20).isActive = true
